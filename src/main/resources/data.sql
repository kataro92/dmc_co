...
create or replace view dmc_warehouse_status as
  ...
  WHERE dmi.id = dmid.material_import_id
        AND dmid.material_group_id = ms.id
        AND (mg.id) :: text = (ms.material_group_code) :: text
        AND dmi.status = 1
  ...
  SELECT dmi.warehouse_id,
            ...
            FROM dmc_material_import dmi,
              dmc_material_import_detail dmid,
              dmc_product_group mg,
              dmc_product_subgroup ms, dmc_product ma
            WHERE dmi.id = dmid.material_import_id
                  AND dmid.product_group_id = ms.id
                  AND (mg.id) :: text = (ms.product_group_code) :: text
                  AND dmi.status = 1
                  AND dmid.product_id = ma.id
            GROUP BY dmi.warehouse_id, dmi.category_id, (to_char(dmi.import_date, 'yyyyMMdd' :: text)), mg.id, ms.id,
              dmid.product_id, ma.name
  UNION ALL
